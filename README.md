...
- Создать и активировать виртуальное окружение```python -m venv .venv```<br>
...
- Создать суперпользователя через кастомную команду ```python manage.py csu```<br>
- Запустить проект через команду ```python manage.py runserver```<br>
- Запустить django-apscheduler командой ```python manage.py scheduler_run``` 
  для работы в фоновом режиме. Чтобы остановить django-apscheduler, введите команду ```Ctrl+C``` в терминале<br>
